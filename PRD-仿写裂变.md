...
> 子文档：[PRD-自定义裂变方向.md](./PRD-自定义裂变方向.md)（FF8 扩展）  
...
| FF8 | 裂变方向 | 条件 | 仅「仅裂变 / 仿写&裂变」；Auto / 场景风 / 实拍风 / **自定义**（见 [PRD-自定义裂变方向.md](./PRD-自定义裂变方向.md)） |
...
每个方向包含：**名称 + 描述 + 步进器（0~10）**；**自定义方向**另含文本输入框与 info 说明（详见 [PRD-自定义裂变方向.md](./PRD-自定义裂变方向.md)）。

| 方向 | 描述文案 | 默认值 | 接口字段 |
|------|----------|--------|----------|
| Auto | 系统自动分配裂变风格 | 2 | **待确认**（见 §9.2） |
| 场景风 | 生活化/主题化场景，氛围感强 | 0 | `sceneFissionNum` |
| 实拍风 | 真实场景/上身实拍，突出商品质感 | 0 | `solidFissionNum`（原「纯色风」UI 已调整，字段名待确认） |
| **自定义裂变方向** | 用户填写卖点/表现指令 | 0 | **待确认**（见 [PRD-自定义裂变方向.md](./PRD-自定义裂变方向.md) §6） |

**校验（裂变相关模式）：** Auto + 场景风 + 实拍风 + **自定义** 四者之和必须 **> 0**。

**预计产出摘要（FF9）：**

| 素材类型 | 文案模板 |
|----------|----------|
| 单品单图(101) | `预计产出约 {total} 张（Auto {auto} + 场景 {scene} + 实拍 {solid} + 自定义 {custom}）` |
| 多品多图(104) | `预计产出 {total} 套九宫格（每套 9 张 · Auto {auto} + 场景 {scene} + 实拍 {solid} + 自定义 {custom}）` |

- `total` = 四向步进器之和；多品下 **1 组方向 = 1 套九宫格**。
...
### 9.5 自定义裂变方向（customFissionNum / customFissionPrompt）

- **UI 要求**：第 4 种裂变方向；步进器定组数 + 文本框填**一条完整指令**（不按标点拆分）。详见 [PRD-自定义裂变方向.md](./PRD-自定义裂变方向.md)。
- **接口现状**：无对应字段。
- **待确认**：建议 `customFissionNum` + `customFissionPrompt`（单字符串，非列表）。
