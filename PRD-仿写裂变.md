...
| FF4 | 参考图 URL | 是 | 单品单行；多品多 URL，支持预览 |
...
| FF9 | 预计产出摘要 | - | 裂变模式下实时汇总 Auto+场景+纯色数量 |
| FF10 | 提交生成 | - | 校验 → submitTask → 轮询 queryTaskDetail → 多图结果展示 |
| FF11 | 任务历史回溯 | - | 回填仿写裂变全部配置字段 |
| FF12 | 重新生成 | - | 复用当前表单参数再次提交 |
...
- 文案：`预计产出约 {total} 张（Auto {auto} + 场景 {scene} + 纯色 {solid}）`
- `total = auto + scene + solid`（UI 估算，最终以接口返回明细为准）
...
- 一次任务可能返回多条成功明细，中间面板须支持**多图网格**展示，而非单图。
