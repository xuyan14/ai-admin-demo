...
> 子文档：[PRD-裂变思路.md](./PRD-裂变思路.md)（FF8 扩展）  
> 接口文档：[外投图-仿写裂变接口文档.md](./外投图-仿写裂变接口文档.md)  
> 原型参考：`creative-tools-prototype.html`（万能指令 → 仿写裂变 Tab）  
> 文档说明：描述仿写裂变能力的功能要求、表单逻辑、接口映射与正式版开发约束；视觉风格对齐 Arco Design
...
| **裂变** | 在商品信息不变的前提下，生成多种视觉风格变体（Auto / 实拍风 / 模版化） |
...
| FF8 | 裂变方向 | 条件 | 仅「仅裂变 / 仿写&裂变」；Auto / 实拍风 / 模版化（见 §4.8） |
| FF8b | 裂变思路 | 条件 | 选填；独立于裂变方向，见 [PRD-裂变思路.md](./PRD-裂变思路.md) |
...
每个方向包含：**名称 + 描述 + 步进器（0~10）**。面板下方另设独立选填项「**裂变思路**」（详见 [PRD-裂变思路.md](./PRD-裂变思路.md)）。
...
**校验（裂变相关模式）：** Auto + 实拍风 + 模版化 三者之和必须 **> 0**。裂变思路为选填，不参与该校验。

**预计产出摘要（FF9）：**

| 素材类型 | 文案模板 |
|----------|----------|
| 单品单图(101) | `预计产出约 {total} 张（Auto {auto} + 实拍 {scene} + 模版 {solid}）` |
| 多品多图(104) | `预计产出 {total} 套九宫格（每套 9 张 · Auto {auto} + 实拍 {scene} + 模版 {solid}）` |

- `total` = 三向步进器之和；多品下 **1 组方向 = 1 套九宫格**。
...
| 实拍风数量 | `sceneFissionNum` | int | 仅仿写时传 0 |
| 模版化数量 | `solidFissionNum` | int | 仅仿写时传 0 |
| 裂变思路 | `fissionThinkingText` | string | 选填；见 [PRD-裂变思路.md](./PRD-裂变思路.md) |
| Auto 数量 | — | — | **待确认**，见 §9.2 |
| 生成数量 | — | — | **待确认**，见 §9.1 |
...
示例：多品 + 裂变，Auto=2、实拍=1、模版=1 → **4 套**九宫格，中间面板纵向排列 4 个 3×3 区块，组标题如「Auto 裂变 1」「实拍风」等。
...
3. 每组 `label` 含方向名与序号（如「Auto 裂变 2」「实拍风」）。
...
| 实拍风数量 | `fissionSceneNum`: number |
| 模版化数量 | `fissionSolidNum`: number |
| 裂变思路 | `fissionThinkingText`: string |
...
### 9.5 裂变思路（fissionThinkingPrompt）

- **UI 要求**：独立于三种裂变方向；单行文本、选填。详见 [PRD-裂变思路.md](./PRD-裂变思路.md)。
- **接口现状**：无对应字段。
- **待确认**：建议 `fissionThinkingPrompt`（单字符串，任务级，作用于所有裂变方向组）。
...
| 素材类型 | 未描述 | **新增 101 / 104** |
| 裂变方向 | 未描述 | **Auto / 实拍风 / 模版化 + 步进器** |
| 裂变思路 | 未描述 | **独立选填字段**，见 [PRD-裂变思路.md](./PRD-裂变思路.md) |
...
| 2026-05-19 | v1.3 | 裂变方向收敛为 Auto / 实拍风 / 模版化；「裂变思路」独立选填；子文档 [PRD-裂变思路.md](./PRD-裂变思路.md) |
